...
**USAA CSV format**: Headers on line 1: `Date,Description,Original Description,Category,Amount,Status`. `Status` column is ignored. Description is formed by concatenating `Description` and `Original Description` with a space (skipping either if null). Amounts are already system-signed (negative = debit, positive = credit); type is inferred from sign. `Category` is used directly. Date format `%Y-%m-%d` (ISO). No `skiprows` needed.
